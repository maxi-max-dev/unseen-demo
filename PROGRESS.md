...
# 批次 K · v0.9 止血（2026-07-30）

## 任务 0 回执
- 基线核对：`git log --oneline -1` = `011ca68` ✅（工作区干净）；`node tools/acceptance.mjs` 无参 = `shot | sweep | walk | kill` ✅
- 目标：按《UNSEEN-技术总纲-2026-07-30》第六章账本做 v0.9 止血，把「看起来能用其实是坏的」修掉；规格已写死，我只执行不重新设计。
- 施工顺序：①上传许可自动续签 ②删 yaw 假插值 ③私密空间发布断言 ④假入口标演示 ⑤任务墙改心愿口径 ⑥小程序达标（可自动化验证部分）⑦账本回标。每件收工即 commit + push。
- 最大风险：
  1. 第②件动的是 AI 主链路判定口径，修后 yaw 会与历史值不同（历史值本就是错的，但线上快照会肉眼「变了」）；靠固定夹具断言 + s4 只读不写兜底。
  2. 第⑥件 b 项要 worker 在发布链路里生成降档全景，最容易误伤 s4；每次动过发布链路必须核对 s4 photos 数并记数。
  3. 真机测试被明令不做，小程序六条只能验到模拟器 + 预览码层，真机风险如实留 BLOCKED.md。

## 施工 1 · 上传许可自动续签（P0-2）✅

**改哪儿**：`server/worker.py` 新增 `upload_policy_state()` / `renew_upload_policy_if_needed()`，接在主循环每轮开头（`run_forever`）和 `--once` 路径。剩余有效期不足 24h 就自动重发一次快照，顺手把 Policy 续上；失败记 5 分钟冷却，不刷屏。

**没改 publish.py**：本来想在发布回写事务里记一笔到期时刻，动手才发现 `publish.py:623` 早就写了 `uploadExpiresAt`（位置比我想加的更好，就在 manifest PUT 的同一个事务里）。我那条是重复，已撤，publish.py 本次零改动。工人直接读这个现成字段，每轮零 OSS 往返。

**一个坑**：`wanted` 必须现算（收集开着 + 有节点 + 名额没满），不能拿快照里的 `expiresAt` 直接判。收集关掉时 `publish.py` 塞的是 `expiresAt = time.time()` 占位值，永远显示"已过期"，照着续会让工人对着一个关掉的空间每轮重发一次。

**手动刷新两个空间**（正式 48h 参数）：
```
======================== s4
刷新前 剩余 33.9 小时
[12:48:44] 已重新发布 → 新传 1 个文件, 跳过 28 个, 耗时 2.05s
刷新后 剩余 48.0 小时  到期=08-01 12:48
======================== stressexp1
刷新前 剩余 33.9 小时
[12:48:46] 已重新发布 → 新传 1 个文件, 跳过 6 个, 耗时 2.0s
刷新后 剩余 48.0 小时  到期=08-01 12:48
```

**反向验证**（把有效期缩到 4 分钟、阈值缩到 2 分钟，只在 stressexp1 上做，不碰 s4）：
第一次试的猴补丁没生效 —— `publish.UPLOAD_EXPIRE_S` 是 `build_public_space` 的默认参数，定义时就绑死了，改模块常量白改（日志里许可照样签 172798s）。真正的签发口子是 `oss.post_policy`，从那里拦才算数。

```
【临时改】许可有效期 -> 240s, 续签阈值 86400s -> 120s
--- 先发一次, 拿到一份 4 分钟的短命许可 ---
[12:55:16] 已重新发布 → 新传 1 个文件, 跳过 6 个, 耗时 1.41s
起点: 剩余 239s
--- 开始每 20 秒巡一轮(模拟工人主循环) ---
[+    0s] 剩余    239s (>阈值, 预期不动手)
[+   20s] 剩余    219s (>阈值, 预期不动手)
[+   40s] 剩余    199s (>阈值, 预期不动手)
[+   60s] 剩余    179s (>阈值, 预期不动手)
[+   80s] 剩余    159s (>阈值, 预期不动手)
[+  100s] 剩余    139s (>阈值, 预期不动手)
[+  120s] 剩余    119s (<阈值, 预期自动续)
[12:57:16] 🔑 直传许可只剩 0.0 小时, 自动续签中…
[12:57:18] 已重新发布 → 新传 1 个文件, 跳过 6 个, 耗时 1.42s
[12:57:18] ✅ 直传许可已续到 07-30 13:01:16(还剩 0.1 小时)
          ↑ 第 1 次自动续签完成, 剩余回到 239s
[+  141s] 剩余    219s (>阈值, 预期不动手)
   …（同样 6 轮不动手）…
[+  242s] 剩余    119s (<阈值, 预期自动续)
[12:59:18] 🔑 直传许可只剩 0.0 小时, 自动续签中…
[12:59:19] ✅ 直传许可已续到 07-30 13:03:18(还剩 0.1 小时)
          ↑ 第 2 次自动续签完成, 剩余回到 239s

结论: 全程自动续签 2 次
【已还原】续签阈值 86400s, post_policy 已复原
[12:59:41] 已重新发布 → 新传 1 个文件, 跳过 6 个, 耗时 1.69s
收尾: 剩余 48.0 小时
```
...
## 施工 4 · 假入口治理（P0-1 / P2-1）✅

**四页加演示条**：`web/studio-login.html`、`app/create.html`、`app/invite.html`、`app/scene.html` 页顶各加一条琥珀色横幅「演示功能 · 正式版建设中」。**一个页面没删、一行代码没删**，只加了 6 行 HTML。每条横幅写的是**这一页各自的真实原因**，不是套话：
- studio-login：这是演示门禁不是账号系统（前端只收 4 位且只认 1111，后端反过来要求 ≥6 位并拉黑 1111，两边互斥）
- create：要连**你自己电脑上的**本机后端才能真建空间，没有后端时只在浏览器里存草稿
- invite：二维码是真的，但角色权限只是界面演示
- scene：编辑操作要连本机后端，否则只显示本地草稿

**顺手修了一句自相矛盾的文案**：studio-login 自己的导语原本写着「进入照片收集、人工审核、策展与发布的完整主办方界面」，跟刚加的演示条正好打架。改成现状表述。

**portal.html 收口**：原来是「02 · LIVE DEMO」+「03 · PAGE INDEX」两节，固定演示、旧模型样例和工作台混排在产品入口旁边。现在合并成一节 **「02 · SHOWCASE 作品演示」**，区块导语写明「全部是演示内容，不代表当前可交付的功能」，六张卡每张右上角戴一枚「演示」牌子。另外两处：
- 首屏最大那颗按钮原来是「进入路演现场」（路演页写死 s900003 的固定演示），现在主按钮给产品入口，路演收进作品演示区块。
- 主办方 Studio 角色卡原本承诺「收集/审核/策展/发布…主办方从这里完成发布」，改成「目前只有演示登录页，后面的工作台还在建设中」，角标从「云端演示登录」改成「演示 · 建设中」。

**顺带断了一个死循环**（P0-1 那条）：工作台 → 点 Studio → 登录页 → 立刻被弹回工作台 → 再点 → 再弹，人被夹在两页之间，既进不去也没人告诉他为什么。根因是 studio-login 的 `pageshow` 无条件 `location.replace(safeNext())`。只改这一处：**从工作台过来时不再自动跳转，停下并明说一句**；登录成功那一跳原样保留。`workspace.html` 不在地界里，所以从我能改的这一头解。

### 验收

**新建两份验收清单**（仓库原来没有，`sweep`/`walk` 都要 JSON 参数）：`tools/sweep-public.json`、`tools/walk-host-loop.json`，跑法写在 `tools/sweep-public.md`。

**sweep 全绿，13/13**：
```
$ .venv/bin/python -m http.server 8791 --bind 127.0.0.1      # 另开一个
$ node tools/acceptance.mjs sweep tools/sweep-public.json
✅ 01 首页转发桩   ✅ 02 产品门户    ✅ 03 宾客上传 s4  ✅ 04 只读展览 s4
✅ 05 3D 漫游演示  ✅ 06 六幕演示    ✅ 07 POV 说明     ✅ 08 路演现场
✅ 09 空间工作台   ✅ 10 演示登录    ✅ 11 创建空间     ✅ 12 邀请共创
✅ 13 场景详情
合计 13 页, 不合格 0 页          （每页 报错=0 死链=0 溢出=false）
```

**每个公开按钮都点得到真页面**：把 portal.html 里全部 href 抽出来逐个请求，16 条全通（4 个锚点 + 12 个页面/资源，全 200，零 404）。

**死循环回归**：
```
$ node tools/acceptance.mjs walk tools/walk-host-loop.json
02 点主办方 Studio 卡 → /web/studio-login.html | 演示条=true
04 提交(1111)        → /workspace.html
05 从工作台再点 Studio → /web/studio-login.html
06 判定 → ✅ 停在登录页, 循环已断 | 提示语: 你已经通过演示门禁了。可操作的主办工作台还在建设中，这一页暂时没有下一步。
```

**肉眼看过画面**（不只看日志）：四条演示条都在页顶显眼位置、不压任何原有元素；作品演示区块的 DEMO 标签和六张卡的「演示」牌子都渲染正常。

⚠️ **截图不进仓库**：21 张共 9MB，太重，全部可一条命令重生成（落 `tools/shots-k/`）。`.gitignore` 不在地界里加不了忽略规则，所以本地会一直显示 untracked，已记 BLOCKED.md K-2。

⚠️ **自己踩了一次**：收尾那次提交用了 `git add -A`，把这 9MB 截图全扫进去了，跟这一段写的话正好相反。已用 `git rm --cached` 从跟踪里移除并重推；**但 blob 已经进过一次历史**，改写已推送的历史风险大于收益，没动。下次动这个仓库记得别在有大批未跟踪产物时用 `git add -A`。

⚠️ **走查时看到但没修的**（不在本件范围，账本里属 v1.1）：静态站上点「验证并进入」时 `POST /api/host/login` 返回 501，studio-login 的「请求失败就放行」把它当成登录成功。这正是 P0-1 的「失败放行」，技术总纲给它的档期是 v1.1 重做，v0.9 的活是「藏/标演示」，已做到。

**三条基线**：sweep **13/13 全绿**（本件建立基线）｜s4 photos = **28** ｜密钥 grep = **0**。

## 施工 5 · 任务墙改内容心愿口径 ✅

规格（技术总纲第 5 行）：任务墙对宾客说的是**「想要什么照片」**，不是「缺哪个角度」。「缺口」是系统内部怎么算出这条任务的，宾客不关心，说了还像在派活。

**改了两层，因为一层不够**：
1. **生成端** `server/space.py`：`_gap_brief()` 和自动任务的 `title`。这是新任务的源头。
   - 标题：`缺这个角度` → `还想要这边的照片`
   - 空空间：`这个空间还没有任何照片,站在原地朝X先拍一张,把这里撑起来` → `这里还一张照片都没有,想要一张朝X的,站在原地转过去拍就行`
   - 有缺口：`站在原地转向X,拍那个方向` → `想要一张朝X的照片,站在原地转过去把那边拍下来`
2. **展示端** `web/join.html`：光改生成端不够 —— **实测发现**改完之后 s4 的任务卡标题**仍然显示「缺这个角度」**，因为那是换口径之前就写进 space.json 的字符串，生成端只管以后新建的。所以在展示层加了 `wishText()`，把这几句**自动生成的固定句式**翻译过来。space.json 一个字节没改，s4 里存的还是老字符串（下面有证据）。
   ⚠️ 只认那几句固定句式，主办方自己写的心愿任务（`type !== "gap"`）和任何认不出来的文案一律原样显示，绝不去改人家写的字。

**宾客侧文案全量**：
| 位置 | 旧 | 新 |
|---|---|---|
| 任务墙标题 | 还缺这些 / 今天的拍摄清单 | 大家想要这些照片 |
| 副标题 | 挑一个主题去拍,或者直接交一张现场照片 | 比如新娘母亲落泪的瞬间、带宠物的合影。挑一张你拍过的,或者直接交一张现场照片 |
| 任务卡徽章 | 🧭 空间任务 · 系统发现的空洞 | 🧭 系统心愿 · 这边还没有照片 |
| 任务卡标题 | 缺这个角度 | 还想要这边的照片 |
| 选图弹层 | 拍好了直接选,或者从相册里翻一张 X 的照片 | 想要的是 X 那边的画面。从相册里翻一张,或者现在去拍 |
| 空态 | 去任务墙挑一个,一分钟的事 | 去看看大家想要什么,一分钟的事 |
| 全清空时 | 全部任务已完成 / 所有任务都被认领了 | 想要的都收齐了 / 清单上的都有人交了 |

演示兜底数据里 5 处旧标题一并换掉（demo 模式下宾客也会看到）。

**只改文案没改判定**：`git diff web/join.html` 过滤出所有涉及 `yawRange` / `type ===` / `status` / 条件分支的行，**零命中**。认领仍然按 `yawRange` 整个区间收，`_task_accepts_yaw` 一个字没动。

**验收**（浏览器真跑，不是看代码）：
```
$ node tools/acceptance.mjs walk …（填昵称 → 进任务墙 → 读第一张 gap 卡的全部文字）
🧭 系统心愿 · 这边还没有照片 | 还想要这边的照片 |
想要一张朝左前方的照片,站在原地转过去把那边拍下来 | 300° 左前方 | +50分 | 我去拍这张
```
改之前同一条命令读出来的是：`… | 缺这个角度 | 站在原地转向左前方,拍那个方向 | …`（这就是"只改生成端不够"的实测证据）。

**数据没被动**：
```
新生成的文案(空空间): 这里还一张照片都没有,想要一张朝左前方的,站在原地转过去拍就行
新生成的文案(有缺口): 想要一张朝右侧的照片,站在原地转过去把那边拍下来
s4 存的任务标题: ['缺这个角度', '缺这个角度', '缺这个角度']   ← 原样躺着,只是不再端给宾客看
```

**同源检查**：小程序这版没有任务墙（`miniapp/utils/upload.js` 全部走自由投稿），所以本件不涉及两端同步。`app/contract.md:141` 拿「缺这个角度」当 title 的示例，现在过时了，但 contract.md 不在地界里，已记 BLOCKED.md。

**三条基线**：sweep **13/13 全绿**（重跑）｜s4 photos = **28** ｜密钥 grep = **0**。

## 施工 6 · 小程序达标（可自动化验证的部分）✅

规格是技术总纲第三章「小程序 v1.0 达标七条」里能自动化的那几条。逐条如下。

### a) sid 从进入页贯穿三页

**真正的坑不在读，在跳**。三页原来各写一句 `(query.sid) || DEFAULT_SPACE_ID`，看着都对；但进入页的「走进这段记忆」跳 pano 时**压根没带 sid**，于是不管从哪进来 pano 都"兜底"回 s4 —— 数据是别的空间的，背景是 s4 的，正是 P0-3。

改法：`util.resolveSid(query)` 统一读（没带时打一句 warn，线上再出现"怎么又是 s4"时那是唯一线索），`util.pageUrl(page, sid, extra)` 统一拼跳转。回归里有一条静态检查：去 pano/photos 的跳转**没有一处是手拼的**（回首页不算，进入页本来就是 sid 的源头）。

### b) 全景随空间动态加载

**服务端**（`server/publish.py`）：`_ensure_pano_mini()` 在发布时为每个节点生成 2048×1024 降档图，字段 `panoMini`。文件名是 `pano-mini-<源图内容哈希12位>.jpg` —— 哈希取的是**源全景的内容**，不是节点 id 也不是时间戳。理由：路演背景可以在保留同一个 n1 的前提下换图，名字不跟内容变的话客户端缓存和 CDN 都会拿旧图冒充新图，而全景恰恰是"换了必须立刻生效"的东西（照片方位全挂在它上面）。生成失败只打日志不挡发布。

顺带把 `_public_resource_rels()` 加了个可选 `root` 参数，让降档图也进"当前仍被引用"的清单，不会被当孤儿清掉。

**小程序**：`util.panoSourceFor(sid, node)` 取 `node.panoMini`；打包的 `s4-n1.jpg` **降级成 s4 专用离线兜底**，别的空间没有降档图时宁可报错也不拿 s4 的背景冒充（这条有专门断言）。

### c) 照片带按 nodeId 过滤

`util.pickNode` / `util.photosOfNode`。以前是把**整个空间**的照片塞进同一条缩略带，二号节点的照片挂在一号节点的全景上，方向全错。老快照里照片没有 nodeId 字段时归当前节点（那种空间只可能有一个节点），不丢照片。

### d) 消费 photos[] + pending[] 全部终态

批次J 只查 `photos[]`，那里面只有**已入选**的照片，待审/被拒/隔离/名额满永远查不到，于是一律"排队中"、三分钟后统一"AI 还在排队" —— 宾客那张明明被拒了还在让他等，这就是 P1-8。现在 `applySnapshot()` 两个数组一起认。

**文案两端同源**：七条状态词表在 `util.STATE_LABELS`，逐条抄自 `web/join.html` 的 `stateLabel()`。回归里有一条**逐字比对**：拿 join.html 源码正则匹配这七条，零差异才算过。

`needs_review` **不算终态**（主办方随时可能通过），所以下次打开页面还会再核一次。

### e) 回执和待重传队列落 wx 本地存储

回执写 `wx.setStorageSync`，杀进程重开恢复。两个必须说清楚的判断：
1. **临时文件路径不落盘**。`wx.chooseMedia` 给的 tempFilePath 进程一杀就失效，存了下次也是错的，等于骗自己"还能重传"。
2. **待重传要真能传**，所以只在**传失败时**才 `saveFile()` 把压缩后的文件存成持久路径（成功的不占存储，传上去后立刻 `removeSavedFile`）。没有这一步，"待重传队列"就是一句空话。
3. 上次被杀时还卡在"上传中"的，重开后**如实标失败**，不继续显示"上传中"骗人。

### f) 进入页「选个空间看看」预设区

清单在 `util.PRESET_SPACES`（s4 + stressexp1），加空间只改那一处。取代了批次J 那张单独的体验空间卡。

### 验收

**1. 逻辑回归 `tools/test_miniapp_contract.js`（新增，34 条断言全过）**：直接 `require()` 小程序自己的 `utils/*.js`，只给 wx 打垫片，不另起一套"看起来像"的对照实现。跑法 `node tools/test_miniapp_contract.js`。抽几条：
```
✅ 去 pano/photos 的跳转没有一处是手拼的  (零命中)
✅ 文件名带内容哈希(换图必然换名，旧缓存命不中)
✅ s4 的降档图云端可读  HTTP 200, 414KB
✅ 别的空间没有兜底，宁可报错也不拿 s4 的背景冒充(P0-3 的核心)
✅ n1 只拿自己的(缺字段的老照片归当前节点)  p1,p3,p4
✅ rejected → 「没被选上」   ✅ quota_full → 「名额已满」  …七条全过
✅ 没有任何一条被含糊成「还在排队」(P1-8 就是这条)
✅ 七条文案与 web/join.html 逐字同源  (零差异)
✅ 杀进程重开后回执恢复  7 条
✅ 上次没传完的重开后如实标失败，不假装还在传
```

**2. 跨空间交叉核对 `tools/shots-k.js`（新增）**，两个内容完全不同的空间交叉进：
```
✅ 两个空间各自的 sid 都对，没有中途掉回 s4
✅ 标题各是各的：「婚礼 · 那一天」 vs 「体验空间 · 宴会厅」
✅ 两个空间的全景贴图都真的加载完成了
✅ 两张全景是不同的图（不再共用打包的 s4 那张）
✅ 用的都是带内容哈希的云端降档图
✅ 照片带各读各的空间（s4=9 张，体验空间=0 张）
```

**3. 编译 + 预览码**：
```
$ cli preview --project ~/code/spatial-memory/miniapp --qr-format base64 --qr-output …
TOTAL 1.1 MB (1172042 Byte)      ← 微信自己的编译器过了，比 node --check 强一层
```

### 两个踩到的坑，写下来免得下一个人重踩

**坑一：`--qr-format image` 在这个版本的开发者工具里是坏的。** 换任何路径都报 `The QR code output path is invalid or does not exist %s`（`%s` 都没替换掉，是没翻译的模板，跟腾讯云 CLI 那个坑同一种）。**`--qr-format terminal` 和 `base64` 都正常**，说明预览本身是成功的，坏的只是 image 这个输出格式。任务书里写的是 image，这里换成 base64 再自己解成 PNG。

**坑二：模拟器截图【拍不到 WebGL 画布】，"截图看着是空的"证明不了任何事。** 第一版截图里全景是一片纯色，我差点当成自己写坏了。**回头翻批次J 留下的 `ui-check/j-02-pano.png`，一模一样** —— 而批次J 贴的还是本地打包图。所以那是截图 API 的限制，不是回归。既然截图不能当判据，就把结果落进 data：新增 `panoReady` / `panoSrcInUse` 两个字段，贴图 onload 成功才置 true，验收脚本直接断言。**实测两个空间都是 `panoReady: true`，各自用自己那张带哈希的降档图。**

另外 `mp.screenshot()` 在带 WebGL 渲染循环的页面上会**偶发挂死**（实测两次，一次 7 分钟一次 10 分钟没返回，同一份脚本第一次跑却是好的）。已给它加 20 秒上限，挂了就跳过继续跑，不让一个留档动作拖垮整条验收。
