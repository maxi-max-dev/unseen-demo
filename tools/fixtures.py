...
真实影石全景到之前,先用这个生成婚礼旅程四个环节的合成全景:
  ...
  2. 从全景里用透视投影 (equirect -> perspective, FOV 70) 在已知 (node, yaw) 裁 40 张 800x600
     "手机照片"(每节点 10 张),并加扰动(亮度、裁边、色偏)模拟真实拍摄。
...
def main():
    ...
    # 每节点 10 张, 共 40 张。文件名用 w 前缀(wedding),不与 AdventureX 旧素材 p001-p012 冲突,旧素材不删。
    # yaw 分布: 每张 pano 12 个 30 度扇区, 每节点跳过其中 2 个(跳过位置按节点错开, 见 skip 公式),
    # 剩余 10 个扇区各出一张照片, 取扇区中点 + 小幅确定性抖动, 尽量把 12 个方向覆盖满。
    # pitch 用同一个 rng 做 ±10 度的轻微抖动模拟手持(不是绕竖直轴, 不影响 yaw 覆盖逻辑)。
    node_keys = list(SCENES.keys())  # ["jieqin", "chufa", "yishi", "yanxi"]
    PHOTOS_PER_NODE = 10
    N_SECTORS = 12
    plan_rng = random.Random(11)  # 独立种子, 只管 yaw/pitch 抖动, 不影响下面像素扰动的 rng(7)
    plan = []
    for node_idx, node in enumerate(node_keys):
        # 跳过 2 个扇区(错开 5 步, 让四个节点跳过的方向不同), 剩下 10 个扇区各出一张
        skip = {(node_idx * 5) % N_SECTORS, (node_idx * 5 + 6) % N_SECTORS}
        sectors = [i for i in range(N_SECTORS) if i not in skip]
        assert len(sectors) == PHOTOS_PER_NODE
        for sector_idx in sectors:
            base_yaw = sector_idx * 30 + 15  # 扇区中点
            yaw = (base_yaw + plan_rng.randint(-8, 8)) % 360
            pitch = plan_rng.randint(-10, 10)
            plan.append((node, yaw, pitch))
    rng = random.Random(7)
    ground_truth = []
